...
`.trim();
        ...
        if (currentLeaf?.view.getViewType() === "canvas") {
          ...
          submit_button.onclick = async () => {
            ...
            if (this.settings.llm_provider_options[this.settings.llm_provider][this.settings.model].streaming) {
              const stream = await this.llm_call_streaming(this.settings.llm_provider, this.settings.model, conversation, 1);
  ...
  async parseXml(xmlString: string): Promise<any> {
    return CaretPlugin.parseXml(xmlString);
  }
      ...
      for (let i = 0; i < incomingEdges.length; i++) {
        ...
        if (ancestor) {
          ...
          if (convo_total_tokens + contextTokens > this.settings.context_window) {
            new Notice("Exceeding context window while adding ancestor context. Stopping further additions.");
            return;
          }
        ...
        if (front_matter.hasOwnProperty("caret_prompt")) {
          ...
          if (caret_prompt === "parallel" && text) {
            ...
            for (let i = 0; i < prompts.length; i++) {
              ...
              const user_node = await this.createChildNode(canvas, node, x, y, new_node_content, "right", "left");
            ...
            for (let i = 0; i < prompts.length; i++) {
              ...
              // Create a new user node
              const user_node = await this.createChildNode(canvas, current_node, x, y, new_node_content, "right", "left");
              user_node.unknownData.role = "user";
              user_node.unknownData.displayOverride = false;
              const sparkle_config: SparkleConfig = {
                model: prompt_model,
  ...
  async refreshNode(
    ...
    const mergedSparkleConfig = this.mergeSettingsAndSparkleConfig(sparkle_config);
  ...
  async update_node_content(node_id: string, stream: any, llm_provider: string) {
    ...
    if (llm_provider === "openai" || llm_provider === "groq" || llm_provider === "custom" || llm_provider === "openrouter") {
